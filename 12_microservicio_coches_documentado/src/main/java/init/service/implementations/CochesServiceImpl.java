...
public class CochesServiceImpl implements CochesService {
	...
	public void actualizarCoche(Coche coche) {
		...
		if(encontrado!=null) {
			encontrado.setMarca(coche.getMarca());
			encontrado.setModelo(coche.getModelo());
			encontrado.setKilometros(coche.getKilometros());
			encontrado.setPrecio(coche.getPrecio());
			encontrado.setFechaFabricacion(coche.getFechaFabricacion());
		}
	}
	...
	public List<Coche> cochesPorMarca(String marca) {
		return coches.stream()
				.filter(c->c.getMarca().toLowerCase().equals(marca.toLowerCase()))
				.toList();
	}

	@Override
	public List<Coche> cochesPorPrecioMax(double precioMax) {
		return coches.stream()
